...
if __name__ == '__main__':
    ...
    print('Records for search uploaded:', len(record_list))
    try:
    ...
    while k <len(record_list):
        try:
            ...
            res = NCBIWWW.qblast('blastn', 'nt', record.seq, url_base='https://blast.ncbi.nlm.nih.gov/Blast.cgi', hitlist_size=5,alignments=5,descriptions=5,
                                 format_type='XML')
            ...
            print('NEWLISTSEQS____',new_list_seqs)
            for i in range(len(new_list_seqs)-1):
                if (new_list_seqs[i+1][0]-new_list_seqs[i][0])>0.0001:
                    break
                else:
                    temp_list.append(new_list_seqs[i+1])
                    print('TEMP LIST__',temp_list)
            for t in temp_list:
